...
(CAST, ISNULL, NOTNULL, NULL, IS, BETWEEN, ELSE, END, CASE, WHEN, THEN, EXISTS,
 ...
 CURRENT_TIMESTAMP, OR) = map(CaselessKeyword, """CAST, ISNULL, NOTNULL, NULL, IS, BETWEEN, ELSE, 
 END, CASE, WHEN, THEN, EXISTS, COLLATE, IN, LIKE, GLOB, REGEXP, MATCH, ESCAPE, 
 CURRENT_TIME, CURRENT_DATE, CURRENT_TIMESTAMP, OR""".replace(",","").split())
...
identifier_map = defaultdict(set)
def saveloc(s, l, t):
    identifier_map[t[0]].add(l)

def reset_id_map():
    identifier_map.clear()

identifier = ~keyword + Word(alphas, alphanums+"_")
identifier.setParseAction(saveloc)
